			...
			if ( preg_match("/^\d+$/", $tagno) && ($tagno < 10) ) {
				...
				foreach ($subfields as $subfield) {
					if ( strlen($subfield) > 0 ) {
					  $arrayKey = substr($subfield, 0, 1); //JD
					  //$subfield_data[substr($subfield, 0, 1)] = substr($subfield, 1); //original line
					  //create a more deeply nested array to handle repeating subfields
